...
const Accounts = () => {
  ...
  const saveProfile = async () => {
    ...
    if (file) {
      ...
      // profileData.file = file; // URL.createObjectURL(file);
      // profileData.file_name = file.name;
    }
    console.log(1, formdata);

    const response = await save_profile(formdata);
    if (response && (response.status === 200 || response.status === 201)) {
      ...
      setEditing(false);
      store.dispatch(
        dataAction.setAlert({ type: "success", message: "Profile updated" })
      );
    }
  };
      ...
      <Spin spinning={isSaving}>
        {!profile.hasOwnProperty("first_name") && editing === false ? (
          <ColContainer style={{ alignItems: "center", marginTop: "60px" }}>
            ...
            <GreenButton onClick={handleCreateProfile}>
              Create Profile +
            </GreenButton>
          </ColContainer>
        ) : (
              ...
              <RowContainer style={{ justifyContent: "space-between" }}>
                <RowContainer>
                  ...
                  <NameText style={{ color: "grey" }}>{`${
                    profile.first_name ?? ""
                  } ${profile.last_name ?? ""}`}</NameText>
                </RowContainer>
                {!editing && (
                  <GreenButton onClick={handleEditing}>Edit</GreenButton>
                )}
              </RowContainer>
            </FeatureContainer>
            <SizedBox />
              ...
              <ColContainer>
                <RowContainer style={{ justifyContent: "space-between" }}>
                  <ColContainer>
                    ...
                    {!editing ? (
                      <DataContainer>{profile.first_name ?? ""}</DataContainer>
                    ) : (
                      <TextField
                        id="first_name"
                        name="first_name"
                  ...
                  <ColContainer>
                    ...
                    {!editing ? (
                      <DataContainer>{profile.last_name ?? ""}</DataContainer>
                    ) : (
                      <TextField
                        id="last_name"
                        name="last_name"
                ...
                <RowContainer style={{ justifyContent: "space-between" }}>
                  <ColContainer>
                    ...
                    {!editing ? (
                      <DataContainer>{profile.address ?? ""}</DataContainer>
                    ) : (
                      <TextField
                        id="address"
                        name="address"
