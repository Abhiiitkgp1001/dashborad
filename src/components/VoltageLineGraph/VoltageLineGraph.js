...
const VoltageLineChart = ({ graphId, graphTab, num_bms, selectedBmsIndex, binStatus }) => {
        ...
        if (num_bms > 0) {
          for (let j = 0; j < num_bms; j++) {
            ...
            if (Object.keys(voltage).length !== 0) {
              const filteredVoltageData = filterDataByBinStatus(
                voltage["bms " + j],
                binStatus
              );
              ...
              for (let i = 0; i < filteredVoltageData.length; i++) {
                for (let k = 0; k < filteredVoltageData[0].length; k++) {
                  if (filteredVoltageData[i][k] !== undefined) {
                    if (k < updatedData["bms_" + j].length) {
                      updatedData["bms_" + j][k].data.push({
                        x: filteredTimeData[i],
                        y: filteredVoltageData[i][k],
                      });
                    } else {
