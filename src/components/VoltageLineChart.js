...
function VoltageLineChart({ selectedBmsIndex }) {
    ...
    if (
      ...
      if (!chartDefined) {
        ...
        for (let i = 0; i < 16; i++) {
          ...
          let lastValue = lastDataItem?.get("valueY");
          let newValue =
            voltage[`bms ${selectedBmsIndex}`] === undefined
              ? "p"
              : voltage[`bms ${selectedBmsIndex}`][
                  voltage[`bms ${selectedBmsIndex}`].length - 1
                ][i] === undefined
              ? "-"
              : voltage[`bms ${selectedBmsIndex}`][
                  voltage[`bms ${selectedBmsIndex}`].length - 1
